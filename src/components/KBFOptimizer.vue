<template>
  <section class="space-y-7">
    <div class="max-w-3xl"><p class="mb-2 text-sm font-bold uppercase tracking-[0.18em] text-plum">Matching workspace</p><h1 class="text-3xl font-bold tracking-tight text-ink sm:text-4xl">Create better board fellow matches.</h1><p class="mt-3 text-lg leading-8 text-slate-600">Upload ranked preferences, review the data, and produce the best possible student-company assignments.</p></div>
    <ol class="grid gap-3 sm:grid-cols-4" aria-label="Optimizer progress"><li v-for="(step, index) in steps" :key="step" class="rounded-xl border px-4 py-3" :class="index <= currentStep ? 'border-purple-200 bg-purple-50 text-ink' : 'border-slate-200 bg-white text-slate-500'"><span class="mr-2 inline-grid h-6 w-6 place-items-center rounded-full text-xs font-bold" :class="index <= currentStep ? 'bg-plum text-white' : 'bg-slate-200 text-slate-500'">{{ index + 1 }}</span><span class="text-sm font-semibold">{{ step }}</span></li></ol>
    <article class="card overflow-hidden">
      <div class="border-b border-slate-100 bg-gradient-to-r from-white to-purple-50 px-5 py-5 sm:px-7"><div class="flex flex-wrap items-start justify-between gap-4"><div><p class="text-sm font-bold text-plum">Step 1 · Add rankings</p><h2 class="mt-1 text-xl font-bold text-ink">Paste a rankings matrix</h2><p class="mt-1 max-w-2xl text-sm leading-6 text-slate-600">Use CSV or tab-separated values. The first row contains companies; the first column contains student names.</p></div><button class="secondary-button text-sm" type="button" @click="loadSample">Load sample data</button></div></div>
    ...
    <article v-if="assignments.length" class="card overflow-hidden" aria-live="polite"><div class="border-b border-slate-100 px-5 py-5 sm:px-7"><div class="flex flex-wrap items-start justify-between gap-4"><div><p class="text-sm font-bold text-plum">Step 3 · Review assignments</p><h2 class="mt-1 text-xl font-bold text-ink">Your optimized matches are ready.</h2></div><button class="primary-button" type="button" @click="download">Download CSV <span aria-hidden="true">↓</span></button></div></div><div class="grid gap-px border-b border-slate-100 bg-slate-100 sm:grid-cols-3"><div class="bg-white px-5 py-4"><p class="text-xs font-bold uppercase tracking-wide text-slate-500">Students matched</p><p class="mt-1 text-2xl font-bold text-ink">{{ assignments.length }}</p></div><div class="bg-white px-5 py-4"><p class="text-xs font-bold uppercase tracking-wide text-slate-500">Companies used</p><p class="mt-1 text-2xl font-bold text-ink">{{ matchedCompanies }}</p></div><div class="bg-white px-5 py-4"><p class="text-xs font-bold uppercase tracking-wide text-slate-500">Average preference</p><p class="mt-1 text-2xl font-bold text-ink">#{{ averageRank }}</p></div></div><div class="p-5 sm:p-7"><ResultsKBF :assignments="assignments" /></div></article>
  </section>
</template>
...
const csvRankings = ref(''), errors = ref([]), assignments = ref([]), average = ref(null), isWorking = ref(false)
let worker
...
function compute() { const payload = parseAndValidate(); if (!payload) return; isWorking.value = true; assignments.value = []; worker?.terminate(); worker = new Worker(new URL('../workers/optimizer.worker.js', import.meta.url), { type: 'module' }); worker.onmessage = ({ data }) => { assignments.value = data.assignments; average.value = data.avgSatisfaction; isWorking.value = false; kelloggConfetti(1.5); worker.terminate() }; worker.onerror = () => { errors.value = ['Unable to calculate assignments. Please try again.']; isWorking.value = false }; worker.postMessage(payload) }
